...
def test_other(args):
    ...
    warp1_tensor, warp2_tensor, mask1_tensor, mask2_tensor = loadSingleData(
        data_path=args.path
    )
    if torch.cuda.is_available():
    ...
    s1 = (
        ((warp1_tensor[0] + 1) * 127.5 * learned_mask1[0])
        .cpu()
        .detach()
        .numpy()
        .transpose(1, 2, 0)
    ...
    s2 = (
        ((warp2_tensor[0] + 1) * 127.5 * learned_mask2[0])
        .cpu()
        .detach()
        .numpy()
        .transpose(1, 2, 0)
    ...
    stitched_image = (
        ((stitched_image[0] + 1) * 127.5).cpu().detach().numpy().transpose(1, 2, 0)
    )
