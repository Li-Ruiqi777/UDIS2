...
def train(args):
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        net.train()
        sigma_total_loss = 0.0
        sigma_boundary_loss = 0.0
        sigma_smooth1_loss = 0.0
        sigma_smooth2_loss = 0.0
        ...
        for i, batch_value in enumerate(train_loader):
            ...
            if i % score_print_fre == 0 and i != 0:
                ...
                print(
                    "Training: Epoch[{:0>3}/{:0>3}] Iteration[{:0>3}]/[{:0>3}] Total Loss: {:.4f}   boundary loss: {:.4f}  smooth loss: {:.4f}  diff loss: {:.4f}   lr={:.8f}".format(
                        epoch + 1,
                        args.max_epoch,
                        i + 1,
                        len(train_loader),
                        average_total_loss,
                        average_boundary_loss,
                        average_smooth1_loss,
                        average_smooth2_loss,
                        optimizer.state_dict()["param_groups"][0]["lr"],
                    )
                )

                # visualization
                writer.add_image("inpu1", (warp1_tensor[0] + 1.0) / 2.0, glob_iter)
                writer.add_image("inpu2", (warp2_tensor[0] + 1.0) / 2.0, glob_iter)

                writer.add_image(
                    "stitched_image", (stitched_image[0] + 1.0) / 2.0, glob_iter
                )
                ...
                writer.add_scalar(
                    "average_boundary_loss", average_boundary_loss, glob_iter
                )
                writer.add_scalar(
                    "average_smooth1_loss", average_smooth1_loss, glob_iter
                )
                writer.add_scalar(
                    "average_smooth2_loss", average_smooth2_loss, glob_iter
                )
