...
def get_overlap_loss(input1, input2, batch_outputs_for_train):
    ...
    delta1 = (torch.sum(warped_target_and_mask[:, 0:3, :, :], [2, 3]) - torch.sum(input1 * warped_target_and_mask[:, 3:6, :, :], [2, 3])) / torch.sum(warped_target_and_mask[:, 3:6, :, :], [2, 3])

    input1_balance = input1 + delta1.unsqueeze(2).unsqueeze(3).expand(-1, -1, img_h, img_w)

    delta2 = (torch.sum(warped_reference_and_mask[:, 0:3, :, :], [2, 3]) - torch.sum(input2 * warped_reference_and_mask[:, 3:6, :, :], [2, 3])) / torch.sum(warped_reference_and_mask[:, 3:6, :, :], [2, 3])
    ...
    delta3 = (torch.sum(tps_warped_target, [2, 3]) - torch.sum(input1 * tps_warped_mask, [2, 3])) / torch.sum(tps_warped_mask, [2, 3])
...
def get_inter_grid_loss(overlap, mesh):
    ...
    # 计算2条连续水平边的夹角
    cos_w = torch.sum(
        w_edges[:, :, 0 : grid_w - 1, :] * w_edges[:, :, 1:grid_w, :], 3
    ) / (
        torch.sqrt(
            torch.sum(
                w_edges[:, :, 0 : grid_w - 1, :] * w_edges[:, :, 0 : grid_w - 1, :], 3
            )
        )
        * torch.sqrt(
            torch.sum(w_edges[:, :, 1:grid_w, :] * w_edges[:, :, 1:grid_w, :], 3)
        )
    )
    ...
    cos_h = torch.sum(
        h_edges[:, 0 : grid_h - 1, :, :] * h_edges[:, 1:grid_h, :, :], 3
    ) / (
        torch.sqrt(
            torch.sum(
                h_edges[:, 0 : grid_h - 1, :, :] * h_edges[:, 0 : grid_h - 1, :, :], 3
            )
        )
        * torch.sqrt(
            torch.sum(h_edges[:, 1:grid_h, :, :] * h_edges[:, 1:grid_h, :, :], 3)
        )
    )
    ...
    depth_diff_w = (
        1 - torch.abs(overlap[:, :, 0 : grid_w - 1] - overlap[:, :, 1:grid_w])
    ) * overlap[:, :, 0 : grid_w - 1]
    error_w = depth_diff_w * delta_w_angle
    
    # on overlapping regions
    depth_diff_h = (
        1 - torch.abs(overlap[:, 0 : grid_h - 1, :] - overlap[:, 1:grid_h, :])
    ) * overlap[:, 0 : grid_h - 1, :]
